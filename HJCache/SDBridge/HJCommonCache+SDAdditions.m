...
static NSData * SDHJPluginCacheDataWithImageData(UIImage *image, NSData *imageData) {
...
@implementation HJCommonCache (SDAdditions)

- (id<SDWebImageOperation>)queryImageForKey:(NSString *)key
                                    options:(SDWebImageOptions)options
                                    context:(SDWebImageContext *)context
                                 completion:(SDImageCacheQueryCompletionBlock)doneBlock {
    ...
    void(^queryDiskBlock)(NSData *) = ^(NSData *diskData) {
        ...
        @autoreleasepool {
            ...
            if (image) {
                ...
                if (diskImage) {
                    // Check extended data
                    NSData *extendedData = [HJDiskCache getExtendedDataFromObject:diskData];
                    if (extendedData) {
                        id extendedObject;
...
#pragma clang diagnostic pop
    ...
    if (shouldQueryDiskSync) {
        ...
        // HJDiskCache's completion block is called in the global queue
...
- (void)storeImageToDisk:(UIImage *)image imageData:(NSData *)imageData forKey:(NSString *)key completion:(SDWebImageNoParamsBlock)completionBlock {
    NSData *data = SDHJPluginCacheDataWithImageData(image, imageData);
    if (!data) {
...
#pragma clang diagnostic pop
            ...
            if (extendedData) {
                [HJDiskCache setExtendedData:extendedData toObject:data];
            }
        }
    }
